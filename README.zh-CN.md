...
│   ├── config/                   # 配置管理
│   │   └── settings.py          # Pydantic Settings（基于 .env）
│   ├── api.py                    # FastAPI REST API 服务
│   ├── app.py                    # Streamlit Web UI
│   ├── cli.py                    # 命令行界面
│   ├── rag.py                    # RAG 系统核心
│   ├── hybrid_retriever.py       # 混合检索器（BM25 + 向量）
│   ├── llm_integration.py        # LLM 集成（通过 LiteLLM）
│   ├── conversation_store.py     # 基于 SQLite 的对话持久化
│   ├── conversation.py           # 对话数据模型
│   ├── auth.py                   # JWT / API Key 认证
│   ├── cache.py                  # LRU + 查询缓存系统
│   ├── performance.py            # 性能监控与优化
│   ├── rate_limiter.py           # API 限流
│   ├── token_cost_manager.py     # LLM Token 费用追踪
│   ├── prompt_templates.py       # 系统提示词模板
│   ├── text_splitter.py          # 文档分块策略
│   ├── document_loader.py        # 多格式文档加载器
│   ├── markdown_loader.py        # Markdown 专用加载器
│   ├── metadata_generator.py     # 文档元数据提取
│   ├── document_import_manager.py # 文档导入编排
│   ├── import_docs.py            # 文档导入 CLI 命令
│   ├── evaluate_rag.py           # RAG 质量评估指标
...
| claude-opus-4-7 | Anthropic 最新模型 |
